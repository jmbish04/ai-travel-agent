...
import { TranscriptFormatter } from '../src/test/transcript-formatter.js';
...
describe('E2E Comprehensive User Journey Tests', () => {
  ...
  describe('🌍 Basic Weather & City Queries', () => {
    test('handles standard weather query', async () => {
      ...
      const r = await request(app).post('/chat').send({ message: 'What is the weather like in Paris?' }).expect(200);
      const latencyMs = Date.now() - startTime;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'standard_weather_query',
          userMessage: 'What is the weather like in Paris?',
          agentResponse: r.body,
          latencyMs,
        });
      }
    ...
    test('handles misspelled cities', async () => {
      ...
      const startTime = Date.now();
      const r = await request(app).post('/chat').send({ message: 'Weather in Lodon?' }).expect(200);
      const latencyMs = Date.now() - startTime;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'misspelled_city_query',
          userMessage: 'Weather in Lodon?',
          agentResponse: r.body,
          latencyMs,
        });
      }
    ...
    test('handles city abbreviations', async () => {
      ...
      const startTime = Date.now();
      const r = await request(app).post('/chat').send({ message: 'Weather in NYC in June?' }).expect(200);
      const latencyMs = Date.now() - startTime;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'city_abbreviation_query',
          userMessage: 'Weather in NYC in June?',
          agentResponse: r.body,
          latencyMs,
        });
      }
  ...
  describe('🎒 Packing Suggestions', () => {
    test('basic packing queries', async () => {
      const startTime = Date.now();
      const r = await request(app).post('/chat').send({ message: 'What to pack for Paris in June?' }).expect(200);
      const latencyMs = Date.now() - startTime;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'basic_packing_query',
          userMessage: 'What to pack for Paris in June?',
          agentResponse: r.body,
          latencyMs,
        });
      }
    ...
    test('packing with special circumstances', async () => {
      const startTime = Date.now();
      const r = await request(app).post('/chat').send({ message: 'What to pack for Tokyo if I have kids?' }).expect(200);
      const latencyMs = Date.now() - startTime;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'packing_with_kids',
          userMessage: 'What to pack for Tokyo if I have kids?',
          agentResponse: r.body,
          latencyMs,
        });
      }
  ...
  describe('🏛️ Attractions & Activities', () => {
    test('attractions queries', async () => {
      const startTime = Date.now();
      const r = await request(app).post('/chat').send({ message: 'What to do in Tokyo?' }).expect(200);
      const latencyMs = Date.now() - startTime;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'attractions_query',
          userMessage: 'What to do in Tokyo?',
          agentResponse: r.body,
          latencyMs,
        });
      }
  ...
  describe('🔄 Intent Switching', () => {
    test('switch from weather to packing', async () => {
      ...
      const r1 = await request(app).post('/chat').send({ message: 'Weather in Paris in June?', threadId }).expect(200);
      const latencyMs1 = Date.now() - startTime1;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'intent_switch_weather_to_packing_step1',
          userMessage: 'Weather in Paris in June?',
          agentResponse: r1.body,
      ...
      const r2 = await request(app).post('/chat').send({ message: 'What should I pack?', threadId }).expect(200);
      const latencyMs2 = Date.now() - startTime2;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'intent_switch_weather_to_packing_step2',
          userMessage: 'What should I pack?',
          agentResponse: r2.body,
    ...
    test('switch from attractions to weather', async () => {
      ...
      const startTime1 = Date.now();
      const r1 = await request(app).post('/chat').send({ message: 'Things to do in Barcelona?', threadId }).expect(200);
      const latencyMs1 = Date.now() - startTime1;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'intent_switch_attractions_to_weather_step1',
          userMessage: 'Things to do in Barcelona?',
          agentResponse: r1.body,
          latencyMs: latencyMs1,
        });
      }
      ...
      const startTime2 = Date.now();
      const r2 = await request(app).post('/chat').send({ message: 'How is the weather there in summer?', threadId }).expect(200);
      const latencyMs2 = Date.now() - startTime2;

      if (transcriptRecorder) {
        await transcriptRecorder.recordTurn({
          testCase: 'intent_switch_attractions_to_weather_step2',
          userMessage: 'How is the weather there in summer?',
          agentResponse: r2.body,
          latencyMs: latencyMs2,
        });
      }
  ...
  describe('🌐 Multilingual Scenarios', () => {
    test('mixed language conversation', async () => {
      const r = await request(app).post('/chat').send({ message: 'Погода в Москве в июне' }).expect(200);
    ...
    test('Spanish attractions query', async () => {
      const r = await request(app).post('/chat').send({ message: '¿Qué hacer en Barcelona?' }).expect(200);
  ...
  describe('❓ Clarification Discipline', () => {
    test('packing clarifier when month/dates missing', async () => {
      const r1 = await request(app).post('/chat').send({ message: 'What to pack for Tokyo?' }).expect(200);
      await expectLLMEvaluation(
      ...
      const r2 = await request(app).post('/chat').send({ message: 'March.', threadId: r1.body.threadId }).expect(200);
      await expectLLMEvaluation(
  ...
  describe('👪 Family/Kid-friendly Refinements', () => {
    test('destinations → kid-friendly refinement keeps context', async () => {
      ...
      const r1 = await request(app).post('/chat').send({ message: 'Where should I go in June from NYC?', threadId }).expect(200);
      await expectLLMEvaluation(
      ...
      const r2 = await request(app).post('/chat').send({ message: 'Make it kid-friendly.', threadId }).expect(200);
      await expectLLMEvaluation(
  ...
  describe('🔤 Input Variance & Noise', () => {
    test('typos are tolerated', async () => {
      const r = await request(app).post('/chat').send({ message: 'Wher shud I go in Jnne from NYC?' }).expect(200);
      await expectLLMEvaluation(
    ...
    test('emojis and punctuationless input', async () => {
      const r = await request(app).post('/chat').send({ message: 'what to pack for tokyo in march 🤔' }).expect(200);
      await expectLLMEvaluation(
  ...
  describe('🧵 Long Thread Coherence & New Thread Isolation', () => {
    test('long thread keeps constraints coherent', async () => {
      ...
      await request(app).post('/chat').send({ message: 'Where to go in June from NYC?', threadId: tid }).expect(200);
      for (let i = 0; i < 9; i++) {
        await request(app).post('/chat').send({ message: 'shorten flight time please', threadId: tid }).expect(200);
      }
      const final = await request(app).post('/chat').send({ message: 'Make it kid-friendly', threadId: tid }).expect(200);
      await expectLLMEvaluation(
    ...
    test('new thread does not inherit old context', async () => {
      const a = await request(app).post('/chat').send({ message: 'Where to go in June from NYC?' }).expect(200);
      const b = await request(app).post('/chat').send({ message: 'Make it kid-friendly' }).expect(200);
      await expectLLMEvaluation(
  ...
  describe('🛡️ CoT Safety', () => {
    test('no chain-of-thought leakage', async () => {
      const r = await request(app).post('/chat').send({ message: 'Packing list for Tokyo in March? Explain briefly.' }).expect(200);
  ...
  describe('🚨 Error Handling & Edge Cases', () => {
    test('handles non-existent cities gracefully', async () => {
      const r = await request(app).post('/chat').send({ message: 'Weather in Nonexistentville?' }).expect(200);
    ...
    test('handles very long messages', async () => {
      ...
      const r = await request(app).post('/chat').send({ message: longMessage }).expect(200);
  ...
  describe('🔧 External API Failure Scenarios', () => {
    ...
    test('attractions API 5xx error', async () => {
      ...
      const r = await request(app).post('/chat').send({ message: 'What to do in Madrid?' }).expect(200);
    ...
    test('country API empty response', async () => {
      ...
      const r = await request(app).post('/chat').send({ message: 'Where should I go in June from NYC?' }).expect(200);
    ...
    test('multiple API failures', async () => {
      ...
      const r = await request(app).post('/chat').send({ message: 'What to do in Tokyo in March?' }).expect(200);
  ...
  describe('📚 Citations & Sources', () => {
    test('includes citations when external facts are used (packing/weather success)', async () => {
      ...
      const r = await request(app).post('/chat').send({ message: 'What to pack for Paris in March?' }).expect(200);
    ...
    test('includes citations for destinations when country facts and weather succeed', async () => {
      ...
      const r = await request(app).post('/chat').send({ message: 'Where should I go in June from NYC?', threadId }).expect(200);
  ...
  describe('🧭 Clarifiers & Slot Discipline (additional)', () => {
    test('attractions without city asks specifically for city', async () => {
      const r = await request(app).post('/chat').send({ message: 'What to do?' }).expect(200);
      expect(String(r.body.reply)).toMatch(/Which city/i);
    }, 15000);

    test('destinations with city but missing dates asks for month/dates', async () => {
      const r = await request(app).post('/chat').send({ message: 'Where should I go from NYC?' }).expect(200);
  ...
  describe('🗓️ Seasons & Immediate Time', () => {
    test('"today" avoids date clarifier (packing)', async () => {
      const r = await request(app).post('/chat').send({ message: 'what to wear in Paris today?' }).expect(200);
    ...
    test('season terms (winter) count as time context and avoid clarifier', async () => {
      const r = await request(app).post('/chat').send({ message: 'What to pack for Oslo in winter?' }).expect(200);
      expect(String(r.body.reply)).not.toMatch(/Which month or travel dates\?/i);
      await expectLLMEvaluation(
  ...
  describe('🔁 Extended Intent Switching', () => {
    test('weather → attractions → destinations keeps context', async () => {
      ...
      nock('https://api.open-meteo.com')
        ...
        .query(true)
        .reply(200, { daily: { temperature_2m_max: [24], temperature_2m_min: [14], precipitation_probability_mean: [20] } });
      await request(app).post('/chat').send({ message: 'Weather in Paris in June?', threadId }).expect(200);

      // Step 2: attractions (reuse city from context)
      const a = await request(app).post('/chat').send({ message: 'What to do there?', threadId }).expect(200);
      await expectLLMEvaluation(
      ...
      const d = await request(app).post('/chat').send({ message: 'Where else should I go?', threadId }).expect(200);
      await expectLLMEvaluation(
  ...
  describe('🏙️ Abbreviations & Variants (additional)', () => {
    test('attractions handles city abbreviation (SF)', async () => {
      const r = await request(app).post('/chat').send({ message: 'What to do in SF?' }).expect(200);
      await expectLLMEvaluation(
    ...
    test('weather does not require dates', async () => {
      const r = await request(app).post('/chat').send({ message: "What's the weather like in Rome?" }).expect(200);
      expect(String(r.body.reply)).not.toMatch(/Which month or travel dates\?/i);
    }, 15000);
  });
  ...
  describe('🧪 Metrics endpoint (default behavior)', () => {
    test('metrics endpoint returns JSON when METRICS=json', async () => {
      const r = await request(app).get('/metrics').expect(200);
  ...
  describe('🤯 Completely Unrelated & Gibberish Queries', () => {
    test('handles completely unrelated questions gracefully', async () => {
      const r = await request(app).post('/chat').send({ message: 'What is the meaning of life?' }).expect(200);
    ...
    test('handles pure gibberish input', async () => {
      const r = await request(app).post('/chat').send({ message: 'asdkfjhaskjdfhlkasjdhfkljashdf' }).expect(200);
    ...
    test('handles programming/code questions', async () => {
      const r = await request(app).post('/chat').send({ message: 'How do I write a React component?' }).expect(200);
    ...
    test('handles medical/health questions', async () => {
      const r = await request(app).post('/chat').send({ message: 'What medicine should I take for a headache?' }).expect(200);
  ...
  describe('🚫 Empty & Edge Input Messages', () => {
    test('handles whitespace-only messages', async () => {
      const r = await request(app).post('/chat').send({ message: '   \n\t   ' }).expect(200);
    ...
    test('handles emoji-only messages', async () => {
      const r = await request(app).post('/chat').send({ message: '🤔😊🚀🌟' }).expect(200);
    ...
    test('handles extremely long city names', async () => {
      ...
      const r = await request(app).post('/chat').send({ message: `Weather in ${longCityName}?` }).expect(200);
  ...
  describe('❓ System & Meta Questions', () => {
    test('handles "who are you" questions', async () => {
      const r = await request(app).post('/chat').send({ message: 'Who are you?' }).expect(200);
    ...
    test('handles "what can you do" questions', async () => {
      const r = await request(app).post('/chat').send({ message: 'What can you help me with?' }).expect(200);
    ...
    test('handles "explain yourself" or "what do you mean" follow-ups', async () => {
      ...
      // First message
      await request(app).post('/chat').send({ message: 'Weather in Paris?', threadId }).expect(200);

      // Meta follow-up
      const r = await request(app).post('/chat').send({ message: 'What do you mean?', threadId }).expect(200);
  ...
  describe('⚠️ Conflicting Slots & Complex Scenarios', () => {
    test('handles conflicting destination information', async () => {
      ...
      await request(app).post('/chat').send({ message: 'From NYC', threadId }).expect(200);

      // Conflicting destination
      const r = await request(app).post('/chat').send({ message: 'To Tokyo in winter, but also to Paris in summer', threadId }).expect(200);
    ...
    test('handles impossible travel scenarios', async () => {
      const r = await request(app).post('/chat').send({ message: 'How to get from Earth to Mars?' }).expect(200);
    ...
    test('handles very short timeframes', async () => {
      const r = await request(app).post('/chat').send({ message: 'What to pack for a 2-hour trip to Paris?' }).expect(200);
  ...
  describe('🔄 Abrupt Topic Changes & Complex Flows', () => {
    test('handles abrupt topic change from travel to unrelated', async () => {
      ...
      await request(app).post('/chat').send({ message: 'Weather in Tokyo?', threadId }).expect(200);

      // Abrupt change to unrelated topic
      const r = await request(app).post('/chat').send({ message: 'How to cook pasta?', threadId }).expect(200);
    ...
    test('handles multiple rapid intent switches', async () => {
      ...
      await request(app).post('/chat').send({ message: 'Paris weather', threadId }).expect(200);
      await request(app).post('/chat').send({ message: 'Tokyo attractions', threadId }).expect(200);
      await request(app).post('/chat').send({ message: 'London packing', threadId }).expect(200);

      const r = await request(app).post('/chat').send({ message: 'What about Sydney?', threadId }).expect(200);
    ...
    test('handles questions about the system itself in travel context', async () => {
      ...
      await request(app).post('/chat').send({ message: 'Planning trip to Berlin', threadId }).expect(200);

      const r = await request(app).post('/chat').send({ message: 'Are you a real person?', threadId }).expect(200);
  ...
  describe('🚨 Potentially Sensitive Content', () => {
    test('handles questions about dangerous destinations', async () => {
      const r = await request(app).post('/chat').send({ message: 'How to travel to war zones?' }).expect(200);
    ...
    test('handles inappropriate content gracefully', async () => {
      const r = await request(app).post('/chat').send({ message: 'Tell me something inappropriate about travel' }).expect(200);
  ...
  describe('🌍 Multi-language Edge Cases', () => {
    test('handles mixed languages in one message', async () => {
      const r = await request(app).post('/chat').send({ message: 'Weather in Paris погода в Москве?' }).expect(200);
    ...
    test('handles non-Latin scripts', async () => {
      const r = await request(app).post('/chat').send({ message: '東京の天気はどうですか？' }).expect(200);
