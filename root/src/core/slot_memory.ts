...
type SlotState = {
  ...
  lastIntent?: 'weather'|'destinations'|'packing'|'attractions'|'unknown'|'web_search';
  lastFacts?: Fact[];
...
export function setLastIntent(threadId: string, intent: 'weather'|'destinations'|'packing'|'attractions'|'unknown'|'web_search'): void {
  const prev = slotStore.get(threadId) ?? { slots: {}, expectedMissing: [] };
  slotStore.set(threadId, { ...prev, lastIntent: intent });
}

export function getLastIntent(threadId: string): 'weather'|'destinations'|'packing'|'attractions'|'unknown'|'web_search'|undefined {
