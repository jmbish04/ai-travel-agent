...
export default {
  async queue(batch: MessageBatch<unknown>, env: ScraperEnv): Promise<void> {
    const controller = await BrowserController.launch(env);

    try {
      await Promise.all(
        batch.messages.map((message) => processMessage(message as Message<QueueMessage>, controller, env)),
      );
    } finally {
